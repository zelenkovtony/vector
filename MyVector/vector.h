...
перегрузка операторов iterator:
	-все булевые сравнени€(сравнивать указатели)
	+операторы инкременции декрименции (постфиксной)

перегрузка операторов vector:
	- все булевые сравнени€ (сравнивать именно значени€, а не указатель)
    + (доступ по индексу к определенному элементу, делаетс€ через указатель)

методы дл€ вектора:
	+ реализовать pop_back
	+ front и back
	+ capacity(количество свободных €чеек на данный момент)
...
{
	//иде€ +- правильна€, но реализовано не правильно
	// данна€ функци€ должна удал€ть ¬—≈ элементы 
	// от _first и до _second
	// а сейчас оно удал€ет лишь _first и _second
	// но ошибка еще в том, что удал€ет 2 элемента, а количество зан€тых €чеек уменьшает на 1
...
{
	//нужно не просто удалить массив, но и выделить пам€ть, поскольку деструктор должен удал€ть пам€ть окончательно
	destroy();
	m_nUsed = 0;
	m_pData = new Type[m_size];
}

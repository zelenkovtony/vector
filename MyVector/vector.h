...
методы front и back
метод capacity(количество свободных ячеек на данный момент)
операторы инкременции декрименции для итератора(постфиксной)
...
public:
	...
	void resize();
	void push_back(Type);
	void pop_back(Type); // реализовать, это изменение последнего элемента в векторе
	iterator begin();
	iterator end();
	size_t size();
	bool empty();
